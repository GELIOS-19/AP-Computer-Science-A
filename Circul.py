...
class Button(object):
    ...
    def is_clicked(self):
        if (
            self.button_x + self.button_width > self.mouse_position[0] > self.button_x
...
class Effect(object):
    ...
    def bubble_pop_effect(self, win):
        ...
        pygame.draw.circle(
            win, self.eff_color, (int(self.eff_x), int(self.eff_y)), self.eff_radius
        )
...
class Player:
    ...
    def draw(self, win):
        ...
        pygame.draw.line(
            win, self.player_color, (400, 300), (self.point_x, self.point_y), 10
        )
...
while Game:
    ...
    for bullet in bullets:
        ...
        for index, bubble in enumerate(bubbles):
            if (
                bubble.hitbox[0] < bullet.x < bubble.hitbox[0] + bubble.hitbox[2]
                and bubble.hitbox[1] < bullet.y < bubble.hitbox[1] + bubble.hitbox[3]
            ):
                ...
                if bubble.radius > 20:
                    ...
                    for i in range(5):
                        eff_bubbles.append(
                            Effect(
                                bubble.x, bubble.y, 0.5, bubble.radius - 8, bubble.color
                            )
                        )
    ...
    if (
        ...
        and bubble.hitbox[0] <= player.hitbox_player[0] + player.hitbox_player[2]
        and bubble.hitbox[1] + bubble.hitbox[3] >= player.hitbox_player[1]
        and bubble.hitbox[1] <= player.hitbox_player[1] + player.hitbox_player[3]
    ):
        print("hi")
