...
public class SoupCarpet extends AdvancedRobot {
  ...
  private static String closestRobot = "";
  private static double closestRobotDistance = Integer.MAX_VALUE;
  ...
  public void run() {
    ...
    setColors(Color.BLACK, Color.WHITE, Color.ORANGE);
  ...
  public void onScannedRobot(ScannedRobotEvent e) {
    if (e.getDistance() <= closestRobotDistance) {
      closestRobot = e.getName();
    }

    if (e.getName() == closestRobot) {
      ...
      GFTWave.targetLocation = GFTUtils.project(
          wave.gunLocation, enemyAbsoluteBearing, enemyDistance);
...
class GFTWave extends Condition {
  ...
  private static final double BIN_WIDTH =
      MAX_ESCAPE_ANGLE / (double) MIDDLE_BIN;

  private static int[][][][] statBuffers =
      new int[DISTANCE_INDEXES][VELOCITY_INDEXES][VELOCITY_INDEXES][BINS];
  ...
  private int currentBin() {
    int bin = (int) Math.round(
        ((Utils.normalRelativeAngle(
             GFTUtils.absoluteBearing(gunLocation, targetLocation) - bearing))
...
class GFTMovement {
  ...
  public void onScannedRobot(ScannedRobotEvent e) {
    ...
    Point2D enemyLocation =
        GFTUtils.project(robotLocation, enemyAbsoluteBearing, enemyDistance);
    Point2D robotDestination;
    double tries = 0;
    while (!fieldRectangle.contains(
               robotDestination = GFTUtils.project(enemyLocation,
                   enemyAbsoluteBearing + Math.PI + direction,
                   enemyDistance * (DEFAULT_EVASION - tries / 100.0)))
        && tries < MAX_TRIES) {
      tries++;
    }
    if ((Math.random()
                < (GFTUtils.bulletVelocity(enemyFirePower) / REVERSE_TUNER)
                    / enemyDistance
            || tries > (enemyDistance / GFTUtils.bulletVelocity(enemyFirePower)
                   / WALL_BOUNCE_TUNER))) {
      direction = -direction;
    }
    // Jamougha's cool way
    double angle = GFTUtils.absoluteBearing(robotLocation, robotDestination)
