...
class WikipediaTopicModel:
  ...
  def get_cos_similarity(self, document1: str, document2: str) -> float:
    ...
    try:
      ...
      embeddings1 = self._topic_model._embed_query(document1)
      embeddings2 = self._topic_model._embed_query(document2)
    except AttributeError:
      # Tokenize each document
      tokenized_document1 = simple_preprocess(strip_tags(document1), deacc=True)
      tokenized_document2 = simple_preprocess(strip_tags(document2), deacc=True)
      # Calculate the embeddings for each document
      embeddings1 = self._topic_model.model.infer_vector(
          doc_words=tokenized_document1,
          alpha=0.025,
          min_alpha=0.01,
          epochs=100)
      embeddings2 = self._topic_model.model.infer_vector(
          doc_words=tokenized_document2,
          alpha=0.025,
          min_alpha=0.01,
          epochs=100)
    # Calculate the cosine similarity with the document embeddings
    cos_similarity = np.dot(embeddings1, embeddings2) / (
