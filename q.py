...
def verify_mypyfile(stub: nodes.MypyFile, runtime: MaybeMissing[types.ModuleType], object_path: List[str]) -> Iterator[
  ...
  for entry in sorted(to_check):
    ...
    yield from verify(stub_entry, getattr(runtime, entry, MISSING), object_path + [entry], )
...
def _verify_static_class_methods(stub: nodes.FuncBase, runtime: Any, object_path: List[str]) -> Iterator[str]:
...
class Signature(Generic[T]):
  ...
  def __str__(self) -> str:
    ...
    def get_desc(arg: Any) -> str:
      ...
      return (get_name(arg) + (": {}".format(arg_type) if arg_type else "") + (" = ..." if has_default(arg) else ""))
    ...
    ret += ", ".join([get_desc(arg) for arg in self.pos] + (
      ["*" + get_name(self.varpos)] if self.varpos else (["*"] if self.kwonly else [])) + [get_desc(arg) for arg in
                       kw_only] + (["**" + get_name(self.varkw)] if self.varkw else []))
    ret += ")"
    return ret
...
Iterator[str]:
  ...
  for stub_arg, runtime_arg in zip(stub.pos, runtime.pos):
    ...
    if (runtime_arg.kind != inspect.Parameter.POSITIONAL_ONLY and stub_arg.variable.name.startswith("__")):
      yield ('stub argument "{}" should be positional or keyword '
...
def verify_funcitem(stub: nodes.FuncItem, runtime: MaybeMissing[Any], object_path: List[str]) -> Iterator[Error]:
  ...
  for message in _verify_signature(stub_sig, runtime_sig, function_name=stub.name):
    yield Error(object_path, "is inconsistent, " + message, stub, runtime, runtime_desc="def " + str(signature), )
...
def verify_var(stub: nodes.Var, runtime: MaybeMissing[Any], object_path: List[str]) -> Iterator[Error]:
  ...
  if (runtime_type is not None and stub.type is not None and not is_subtype_helper(runtime_type, stub.type)):
    ...
    if should_error:
      yield Error(object_path, "variable differs from runtime type {}".format(runtime_type), stub, runtime, )
...
Iterator[Error]:
  ...
  for message in _verify_signature(stub_sig, runtime_sig, function_name=stub.name):
    ...
    if "has a default value of type" in message:
      message += (". This is often caused by overloads failing to account for explicitly passing "
                  "in the default value.")
    yield Error(object_path, "is inconsistent, " + message, stub, runtime, stub_desc=str(stub.type) + "\nInferred signature: {}".format(stub_sig), runtime_desc="def " + str(signature), )
...
def verify_typealias(stub: nodes.TypeAlias, runtime: MaybeMissing[Any], object_path: List[str]) -> Iterator[Error]:
  ...
  if isinstance(stub_target, mypy.types.TupleType):
    if tuple not in getattr(runtime, "__mro__", ()):
      yield Error(object_path, "is not a subclass of tuple", stub, runtime, stub_desc=str(stub_target))
    # could check Tuple contents here...
    return
  if isinstance(stub_target, mypy.types.AnyType):
    return
  yield Error(object_path, "is not a recognised type alias", stub, runtime, stub_desc=str(stub_target))
